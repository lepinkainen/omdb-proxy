...
`omdb-proxy` holds **one** upstream API key and **one** shared cache. Every project talks to the proxy exactly as if it were OMDb itself — same query parameters, same JSON shape, same status codes — so switching a client over is a one-line change, and the proxy only spends real quota on an actual miss. With N projects behind it, a shared library that would cost `N × (library size)` requests spread across `N` separate 1000/day budgets instead costs `1 × (library size)` total, spread across **one** 900/day budget (`DAILY_BUDGET` below): a cold fill of a ~2000-movie library that used to take two days *per project* now takes about two days, once, for every project combined.

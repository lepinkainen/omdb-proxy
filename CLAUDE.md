...
**A served response after a refusal means OMDb's day rolled over.** `RecordServed` carries that rule in one statement: if `exhausted_at` was set, `used` restarts at 1 and `counting_since` moves to now; otherwise `used` increments. This assumes the proxy is the sole spender of its key — if something else shares it, the restart is optimistic and the next miss simply rediscovers exhaustion.

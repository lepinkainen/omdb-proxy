...
CREATE TABLE IF NOT EXISTS quota_epoch (
    ...
    used             INTEGER NOT NULL, -- requests actually spent this epoch
    epoch_started_at TEXT NOT NULL,    -- RFC3339 UTC; last observed upstream rollover
...
The ledger holds exactly one row, and it tracks OMDb's quota day rather than the calendar's. `epoch_started_at` is the last rollover the proxy actually observed — the moment a refused key answered a probe again — because that is the only boundary OMDb makes visible. Absent an observation the epoch rolls itself 24h on, in whole steps so it keeps the hour it was anchored to.

Keying this to UTC dates instead fails in both directions, and both failures are pinned by tests: a breaker latched to the next UTC midnight forfeits a whole day when a quota error lands just after it, and a counter reset at UTC midnight hands out a second full budget inside one upstream day when the epoch began at, say, 06:00.

`Open` drops a pre-existing day-keyed `quota` table rather than migrating it: its rows count spend within a UTC calendar day, which is not what the epoch measures, and rows from before the breaker was separated from the counter hold a forged `used` (the old `MarkExhausted` pushed it to the budget to trip itself). Starting fresh costs at most one epoch of over-spending, which the breaker catches; inheriting a wrong number does not self-correct.
